<template>
  <div class="app-container">
    ...
    >
      <el-table-column label="ID" prop="id" :default-sort = "{prop: 'id', order: 'ascending'}" sortable="custom" align="center" width="80" :class-name="getSortClass('id')">
        <template slot-scope="scope">
          <span>{{ scope.row.recruitId }}</span>
        </template>
      </el-table-column>
      <!-- <el-table-column label="Date" width="150px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.timestamp | ('{y}-{m}-{d} {h}:{i}') }}</span>
        </template>
      </el-table-column> -->
      ...
      <el-table-column class-name="status-col" label="状态" width="110" align="center">
        <template slot-scope="scope">
          <svg-icon v-for="n in + scope.row.departmentState" :key="n" icon-class="star" class="meta-item__icon" />
    ...
    <pagination v-show="total>0" :total="total" :page.sync="listQuery.page" :limit.sync="listQuery.limit" @pagination="getRecruitList" />
